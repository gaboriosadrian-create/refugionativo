...
export class RatePlanRepository extends BaseRepository<RatePlan> {
  constructor() {
    super('pricing_ratePlans');
  }

  private getLocalKey(resortId: string): string {
    return `pricing_rateplans_${resortId}`;
  }

  override async getAll(resortId: string): Promise<RatePlan[]> {
  ...
  override async save(resortId: string, entity: RatePlan): Promise<void> {
    if (isFirebaseConfigured) {
  ...
  public subscribe(resortId: string, callback: (ratePlans: RatePlan[]) => void): () => void {
    if (isFirebaseConfigured) {
      try {
        ...
        const unsubscribe = onSnapshot(colRef, (snapshot) => {
          ...
          Logger.error(`Subscription error for ratePlans in resort ${resortId}, falling back to LocalSaaSDb:`, error);
          this.getAll(resortId).then(callback).catch((err) => {
            Logger.error('Local subscription fallback for ratePlans failed:', err);
          });
        });
        return unsubscribe;
      } catch (error) {
        Logger.error(`Failed to initialize subscription for ratePlans in resort ${resortId}, falling back to LocalSaaSDb:`, error);
        this.getAll(resortId).then(callback).catch((err) => {
          Logger.error('Local subscription fallback for ratePlans failed:', err);
        });
        return () => {};
...
export default ratePlanRepository;
