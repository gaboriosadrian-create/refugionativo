...
export class RevenueMetricsRepository extends BaseRepository<RevenueMetric> {
  constructor() {
    super('revenueMetrics');
  }

  private getLocalKey(resortId: string): string {
    return `revenue_metrics_${resortId}`;
  }

  override async getAll(resortId: string): Promise<RevenueMetric[]> {
  ...
  override async save(resortId: string, entity: RevenueMetric): Promise<void> {
    if (isFirebaseConfigured) {
...
export const revenueMetricsRepository = new RevenueMetricsRepository();
