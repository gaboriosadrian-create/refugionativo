...
export class CommercialCalendarRepository extends BaseRepository<CommercialCalendarEvent> {
  constructor() {
    super('commercialCalendar');
  }

  private getLocalKey(resortId: string): string {
    return `commercial_calendar_${resortId}`;
  }

  override async getAll(resortId: string): Promise<CommercialCalendarEvent[]> {
  ...
  override async save(resortId: string, entity: CommercialCalendarEvent): Promise<void> {
    if (isFirebaseConfigured) {
...
export const commercialCalendarRepository = new CommercialCalendarRepository();
